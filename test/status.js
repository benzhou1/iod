/**
 * Unit test for IOD status request.
 */
			...
			describe('schema validation', function() {
				before(function(callback) {
					...
					async.waterfall([
						...
						function one(done) {
							env.IOD.status(IODOpts, U.beforeDoneFn(env, 'one', done))
						},
						function two(done) {
							IODOpts = { majorVersion: 'invalid majorVersion' }
							env.IOD.status(IODOpts, U.beforeDoneFn(env, 'two', done))
						},
						function three(done) {
							IODOpts = {
							...
							env.IOD.status(IODOpts, U.beforeDoneFn(env, 'three', done))
						},
						function four(done) {
							IODOpts = {
								majorVersion: env.IOD.VERSIONS.MAJOR.V1,
							...
							env.IOD.status(IODOpts, U.beforeDoneFn(env, 'four', done))
						}
					], callback)
				})
					...
					async.waterfall([
						...
						function get(done) {
							...
							if (!jobId) done()
								...
								env.IOD.status(statusIODOpts, U.beforeDoneFn(env, 'get', done))
							}
						},
						function post(done) {
							var jobId = env.async.response && env.async.response.jobID
							if (!jobId) done()
								...
								statusIODOpts.jobId = jobId
								env.IOD.status(statusIODOpts, U.beforeDoneFn(env, 'post', done))
							}
						}
					], callback)
				})
